...
def imprimir_sin_interrumpir_linea(mensaje):
    sys.stdout.write(f"\r{mensaje}\n{NEGRITA}Enviar a (usuario destino) > {RESET}")
    sys.stdout.flush()
        ...
        try:
            ...
            if codigo == CODIGO_MENSAJE:
                if usuario_emisor in usuarios_conectados:
                    print(f"\n{VERDE}[{usuario_emisor} dice]:{RESET} {mensaje}")
                else:
                    imprimir_sin_interrumpir_linea(
                        f"{AZUL}[Solicitud] Conexión de '{usuario_emisor}'. Usá {NEGRITA}/aceptar {usuario_emisor}{RESET}{AZUL} o {NEGRITA}/rechazar {usuario_emisor}{RESET}{AZUL}.{RESET}")
                ...
                imprimir_sin_interrumpir_linea(f"{VERDE}[Conexión aceptada] Ya podés chatear con '{usuario_emisor}'.{RESET}")
                #print(f"\n{VERDE}[Conexión aceptada] Ya podés chatear con '{usuario_emisor}'.{RESET}")
            elif codigo == CODIGO_RECHAZADO:
                print(f"\n{ROJO}[Conexión rechazada] {usuario_emisor} no aceptó la solicitud.{RESET}")
            else:
...
if realizar_conexion():
    ...
    while True:
        ...
        print(f"{AZUL}Conversación con {destino_str}{RESET}")
        entrada = input(f"{NEGRITA}[{USUARIO.decode()} → {destino_str}]{RESET} > ").strip()
        #mandar archivo
        ...
        if entrada.lower() == "salir":
            ...
            nombre_usuario = entrada.split(maxsplit=1)[1].strip()
            usuario_a_aceptar = nombre_usuario.encode('utf-8')[:32]
            ...
            print(f"{VERDE}[+] Aceptaste la conexión con {nombre_usuario}{RESET}")

            # Enviar mensaje vacío para que el servidor active la conexión
            paquete_fake = construir_paquete(CODIGO_MENSAJE, usuario=USUARIO, destino=usuario_a_aceptar, datos=b"")
            socket_cliente.sendall(paquete_fake)
